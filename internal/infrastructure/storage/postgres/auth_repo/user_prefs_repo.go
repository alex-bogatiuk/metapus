...
func (r *UserPrefsRepo) SaveDashboardLayout(ctx context.Context, userID id.ID, data json.RawMessage) error {
	q := r.getTxManager(ctx).GetQuerier(ctx)

	query := `
		INSERT INTO user_preferences (user_id, dashboard_layout)
		VALUES ($1, $2)
		ON CONFLICT (user_id) DO UPDATE
		SET dashboard_layout = $2, updated_at = NOW()
	`
