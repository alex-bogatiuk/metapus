...
func (r *ReportRepo) GetDocumentJournal(ctx context.Context, filter reports.DocumentJournalFilter) (*reports.DocumentJournal, error) {
	...
	unions := make([]string, 0, len(docTypes))
	args := make([]any, 0, len(docTypes)*4)
	argIndex := 1
		...
		case "goods_receipt":
			q := `
				SELECT 
					...
					d.posted,
					d.supplier_id as counterparty_id, COALESCE(cp.name, '') as counterparty_name,
					d.warehouse_id, COALESCE(w.name, '') as warehouse_name,
					0.0 as total_quantity,
					COALESCE((SELECT SUM(amount) FROM doc_goods_receipt_lines WHERE document_id = d.id), 0) as total_amount,
					COALESCE(cur.iso_code, '') as currency, d.description, d.deletion_mark, d.created_at, d.updated_at
				FROM doc_goods_receipts d
				LEFT JOIN cat_currencies cur ON d.currency_id = cur.id
				LEFT JOIN cat_warehouses w ON d.warehouse_id = w.id
				LEFT JOIN cat_counterparties cp ON d.supplier_id = cp.id
				WHERE d.deletion_mark = false
			`

			if filter.FromDate != nil {
				q += fmt.Sprintf(" AND d.date >= $%d", argIndex)
			...
			if filter.Posted != nil {
				q += fmt.Sprintf(" AND d.posted = $%d", argIndex)
		...
		case "goods_issue":
			q := `
				SELECT 
					...
					d.posted,
					d.customer_id as counterparty_id, COALESCE(cp.name, '') as counterparty_name,
					d.warehouse_id, COALESCE(w.name, '') as warehouse_name,
					0.0 as total_quantity,
				...
				LEFT JOIN cat_currencies cur ON d.currency_id = cur.id
				LEFT JOIN cat_warehouses w ON d.warehouse_id = w.id
				LEFT JOIN cat_counterparties cp ON d.customer_id = cp.id
				WHERE d.deletion_mark = false
			`
...
func (r *ReportRepo) GetDocumentTypeSummary(ctx context.Context, filter reports.DocumentJournalFilter) ([]reports.DocumentTypeSummary, error) {
	...
	result := make([]reports.DocumentTypeSummary, 0, len(docTypes))
