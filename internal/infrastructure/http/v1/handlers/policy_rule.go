...
func NewPolicyRuleHandler(repo security_profile.PolicyRuleRepository, engine *security.PolicyEngine, provider security_profile.ProfileProvider) *PolicyRuleHandler {
...
func (h *PolicyRuleHandler) Create(c *gin.Context) {
	...
	// Invalidate all cached profiles (new rule affects users assigned to this profile)
	h.invalidateAllProfiles()
...
func (h *PolicyRuleHandler) Update(c *gin.Context) {
	...
	// Invalidate all cached profiles (updated rule affects users assigned to this profile)
	h.invalidateAllProfiles()
...
func (h *PolicyRuleHandler) Delete(c *gin.Context) {
	...
	// Invalidate all cached profiles (deleted rule affects users assigned to this profile)
	h.invalidateAllProfiles()
...
// invalidateAllProfiles clears the entire profile cache (e.g., after rule CRUD).
func (h *PolicyRuleHandler) invalidateAllProfiles() {
