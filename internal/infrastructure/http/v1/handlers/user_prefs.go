...
// SaveDashboardLayout handles PUT /me/preferences/dashboard-layout
func (h *UserPrefsHandler) SaveDashboardLayout(c *gin.Context) {
	ctx := c.Request.Context()
	...
	if err := h.repo.SaveDashboardLayout(ctx, userID, json.RawMessage(body)); err != nil {
		h.Error(c, err)
		return
	}
