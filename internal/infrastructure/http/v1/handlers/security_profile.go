...
func NewSecurityProfileHandler(repo security_profile.Repository, audit *postgres.AuditService, provider security_profile.ProfileProvider) *SecurityProfileHandler {
	return &SecurityProfileHandler{repo: repo, audit: audit, profileProvider: provider}
}
...
func (h *SecurityProfileHandler) Create(c *gin.Context) {
	...
	// Invalidate all cached profiles (new profile may affect users via future assignment)
	h.invalidateAll()
...
func (h *SecurityProfileHandler) Update(c *gin.Context) {
	...
	// Invalidate all cached profiles (dimensions, field policies, etc. changed)
	h.invalidateAll()
...
func (h *SecurityProfileHandler) Delete(c *gin.Context) {
	...
	// Invalidate all cached profiles (deleted profile no longer applies)
	h.invalidateAll()
...
func (h *SecurityProfileHandler) AssignUser(c *gin.Context) {
	...
	// Invalidate cache for the specific user whose profile assignment changed
...
// invalidateAll clears the entire profile cache (e.g., after profile CRUD).
func (h *SecurityProfileHandler) invalidateAll() {
	if h.profileProvider != nil {
		h.profileProvider.InvalidateAll()
	}
}

// invalidateUser clears a single user's cached profile (e.g., after AssignUser/RemoveUser).
func (h *SecurityProfileHandler) invalidateUser(userID id.ID) {
	if h.profileProvider != nil {
		h.profileProvider.Invalidate(userID)
	}
}
...
func profileSnapshot(p *security_profile.SecurityProfile) map[string]any {
	...
	if p.EntityDimensions != nil {
		edJSON, _ := json.Marshal(p.EntityDimensions)
		m["entityDimensions"] = string(edJSON)
	}
