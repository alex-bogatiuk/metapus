...
// SetRolePermissionsRequest for bulk-replacing role permissions.
...
// refreshToken is now delivered via httpOnly cookie, not JSON.
type TokenResponse struct {
	AccessToken  string    `json:"accessToken"`
	RefreshToken string    `json:"-"` // excluded from JSON — delivered via httpOnly cookie
...
// FromRoleDetailed creates a detailed response with permissions and user count.
func FromRoleDetailed(r *auth.Role, perms []auth.Permission, userCount int) *RoleResponse {
	resp := FromRole(r)
	resp.UserCount = &userCount
	if len(perms) > 0 {
		resp.Permissions = make([]PermissionResponse, len(perms))
		for i := range perms {
			resp.Permissions[i] = *FromPermission(&perms[i])
		}
	}
	return resp
}
