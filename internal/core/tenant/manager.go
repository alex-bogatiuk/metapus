...
type ManagerConfig struct {
	...
	// VersionGroup restricts this instance to serve only tenants with a matching
	// version_group value. Empty string means no filtering (self-hosted mode).
	// In cloud mode, set to the binary version (e.g. "v1.3.0").
	VersionGroup string
}
...
// Config returns the manager configuration (read-only access).
...
func (m *Manager) Close() {
	...
	// Close all pools and remove from map
	var poolsClosed int
	m.pools.Range(func(key, value any) bool {
		mp := value.(*ManagedPool)
		mp.pool.Close()
...
// EvictPool forcibly closes and removes the connection pool for a tenant.
// Used before running schema migrations to release all connections so that
// goose can acquire exclusive DDL locks.
// Safe to call even if no pool exists for the given tenant.
func (m *Manager) EvictPool(tenantID string) {
	val, ok := m.pools.Load(tenantID)
	if !ok {
		return
	}
	mp := val.(*ManagedPool)
	m.closePool(tenantID, mp, "evicted for schema update")
}
...
// ListByVersionGroup returns active tenants for a specific version group.
// Falls back to ListActive if group is empty.
func (m *Manager) ListByVersionGroup(ctx context.Context, group string) ([]*Tenant, error) {
	if group == "" {
		return m.registry.ListActive(ctx)
	}
	return m.registry.ListByVersionGroup(ctx, group)
}
