...
// celTestEntityDualKeys has different db (snake_case) and json (camelCase) tag names,
// matching real domain models like GoodsReceipt.
type celTestEntityDualKeys struct {
	TotalAmount int64  `db:"total_amount" json:"totalAmount"`
	DocStatus   string `db:"doc_status" json:"docStatus"`
}

func TestPolicyEngine_Evaluate_CamelCaseKey(t *testing.T) {
	engine := newTestEngine(t)
	ctx := context.Background()

	rule := makeRule("r1", "doc.totalAmount > 500", "deny", 10)
	entity := &celTestEntityDualKeys{TotalAmount: 1000, DocStatus: "draft"}

	err := engine.Evaluate(ctx, []PolicyRule{rule}, "create", entity)
	if err == nil {
		t.Fatal("expected deny for camelCase key totalAmount > 500 with value 1000")
	}
}

func TestPolicyEngine_Evaluate_SnakeCaseKey(t *testing.T) {
	engine := newTestEngine(t)
	ctx := context.Background()

	rule := makeRule("r1", "doc.total_amount > 500", "deny", 10)
	entity := &celTestEntityDualKeys{TotalAmount: 1000, DocStatus: "draft"}

	err := engine.Evaluate(ctx, []PolicyRule{rule}, "create", entity)
	if err == nil {
		t.Fatal("expected deny for snake_case key total_amount > 500 with value 1000")
	}
}

func TestPolicyEngine_Evaluate_BothKeysConsistent(t *testing.T) {
	engine := newTestEngine(t)
	ctx := context.Background()

	// Both camelCase and snake_case should resolve to the same value
	camelRule := makeRule("r1", "doc.totalAmount > 500", "deny", 10)
	snakeRule := makeRule("r2", "doc.total_amount > 500", "deny", 10)

	entityOver := &celTestEntityDualKeys{TotalAmount: 1000}
	entityUnder := &celTestEntityDualKeys{TotalAmount: 100}

	// Both should deny when over threshold
	if err := engine.Evaluate(ctx, []PolicyRule{camelRule}, "create", entityOver); err == nil {
		t.Fatal("camelCase key should deny when over threshold")
	}
	if err := engine.Evaluate(ctx, []PolicyRule{snakeRule}, "create", entityOver); err == nil {
		t.Fatal("snake_case key should deny when over threshold")
	}

	// Both should allow when under threshold
	if err := engine.Evaluate(ctx, []PolicyRule{camelRule}, "create", entityUnder); err != nil {
		t.Fatalf("camelCase key should allow when under threshold: %v", err)
	}
	if err := engine.Evaluate(ctx, []PolicyRule{snakeRule}, "create", entityUnder); err != nil {
		t.Fatalf("snake_case key should allow when under threshold: %v", err)
	}
}
