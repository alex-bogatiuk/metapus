// Package unit provides the Unit catalog.
...
const (
	TypePiece  UnitType = "piece"  // Pieces
	TypeWeight UnitType = "weight" // Weight (kg, g, t)
	TypeLength UnitType = "length" // Length (m, cm, mm)
	TypeArea   UnitType = "area"   // Area (m², cm²)
	TypeVolume UnitType = "volume" // Volume (l, ml, m³)
	TypeTime   UnitType = "time"   // Time (h, min, sec)
	TypePack   UnitType = "pack"   // Packaging (Packs)
)
...
type Unit struct {
	...
	Type UnitType `db:"type" json:"type" meta:"label:Тип"`
	...
	ConversionFactor decimal.Decimal `db:"conversion_factor" json:"conversionFactor" meta:"label:Коэффициент"`

	// IsBase indicates if this is a base unit (not derived)
	IsBase bool `db:"is_base" json:"isBase" meta:"label:Базовая"`
