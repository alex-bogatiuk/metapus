// Package contract provides the Contract catalog.
...
const (
	TypeSupply ContractType = "supply" // Supply contract (with supplier)
	TypeSale   ContractType = "sale"   // Sale contract (with customer)
	TypeOther  ContractType = "other"  // Other
)
...
type Contract struct {
	...
	CounterpartyID id.ID `db:"counterparty_id" json:"counterpartyId" meta:"label:Контрагент"`

	// Type defines the contract type
	Type ContractType `db:"type" json:"type" meta:"label:Тип"`

	// CurrencyID is the optional currency for this contract
	CurrencyID *id.ID `db:"currency_id" json:"currencyId,omitempty" meta:"label:Валюта"`

	// ValidFrom is the start date of the contract
	ValidFrom *time.Time `db:"valid_from" json:"validFrom,omitempty" meta:"label:Дата начала"`

	// ValidTo is the end date of the contract
	ValidTo *time.Time `db:"valid_to" json:"validTo,omitempty" meta:"label:Дата окончания"`

	// PaymentTermDays is the payment term in days
	PaymentTermDays int `db:"payment_term_days" json:"paymentTermDays" meta:"label:Срок оплаты (дн.)"`
