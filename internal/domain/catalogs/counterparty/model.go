// Package counterparty provides the Counterparty catalog.
...
const (
	TypeCustomer CounterpartyType = "customer" // Customer
	TypeSupplier CounterpartyType = "supplier" // Supplier
	TypeBoth     CounterpartyType = "both"     // Both Customer and Supplier
	TypeOther    CounterpartyType = "other"    // Other
)
...
const (
	LegalIndividual LegalForm = "individual"  // Individual
	LegalSoleTrader LegalForm = "sole_trader" // Sole Trader
	LegalCompany    LegalForm = "company"     // Legal entity (Company)
	LegalGovernment LegalForm = "government"  // Government body
)
...
type Counterparty struct {
	...
	Type CounterpartyType `db:"type" json:"type" meta:"label:Тип"`

	// LegalForm defines the legal status
	LegalForm LegalForm `db:"legal_form" json:"legalForm" meta:"label:Юр. форма"`
	...
	LegalAddress *string `db:"legal_address" json:"legalAddress,omitempty" meta:"label:Юр. адрес"`
	...
	Comment *string `db:"comment" json:"comment,omitempty" meta:"label:Комментарий"`
}
