...
func TestBuildSecurityContext_Admin(t *testing.T) {
	sc := BuildSecurityContext(nil, true)
...
func TestBuildSecurityContext_NoProfile(t *testing.T) {
	// Fail-open: no profile = empty DataScope (no restrictions)
	sc := BuildSecurityContext(nil, false)

	if sc.DataScope.IsAdmin {
		t.Fatal("should not be admin")
	}
	// Empty dimensions = no restrictions (fail-open)
	if len(sc.DataScope.Dimensions) != 0 {
		t.Fatalf("expected empty dimensions for fail-open, got %v", sc.DataScope.Dimensions)
	}
	if sc.FieldPolicies != nil {
...
func TestBuildSecurityContext_WithProfile(t *testing.T) {
	...
	sc := BuildSecurityContext(profile, false)

	// Organization: profile dimensions are the sole source of truth
	orgs := sc.DataScope.Dimensions[security.DimOrganization]
	if len(orgs) != 2 || orgs[0] != "org-2" || orgs[1] != "org-3" {
		t.Fatalf("expected profile orgs [org-2, org-3], got %v", orgs)
	}
...
func TestSecurityProfile_BuildDataScope_NoProfileOrgs(t *testing.T) {
	// Profile has no organization dimension → fail-open (no restrictions)
	profile := &SecurityProfile{
		ID:         id.New(),
		Dimensions: map[string][]string{},
	}

	scope := profile.BuildDataScope(false)

	// No org dimension in profile → no org restriction (fail-open)
	if _, ok := scope.Dimensions[security.DimOrganization]; ok {
		t.Fatal("expected no organization dimension for fail-open")
	}
}
