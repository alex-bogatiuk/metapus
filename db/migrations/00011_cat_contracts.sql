-- +goose Up
-- Description: Contracts catalog (Справочник "Договоры контрагентов")
...
CREATE TRIGGER trg_cat_contracts_updated_at
    BEFORE UPDATE ON cat_contracts
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Keyset pagination
CREATE INDEX idx_cat_contracts_name_id ON cat_contracts (name ASC, id ASC);
...
-- +goose StatementBegin
SELECT pg_advisory_lock(hashtext('metapus_migrations'));
DROP TABLE IF EXISTS cat_contracts CASCADE;
SELECT pg_advisory_unlock(hashtext('metapus_migrations'));
-- +goose StatementEnd
