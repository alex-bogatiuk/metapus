-- +goose Up
-- Description: Goods Issue document (Документ "Расход товаров")
...
CREATE TABLE doc_goods_issues (
    ...
    CONSTRAINT uq_goods_issue_number      UNIQUE (organization_id, number),
...
-- Keyset pagination
CREATE INDEX idx_doc_goods_issues_date_id    ON doc_goods_issues (date DESC, id DESC);
CREATE INDEX idx_doc_goods_issues_created_id ON doc_goods_issues (created_at DESC, id DESC);
