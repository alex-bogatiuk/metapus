...
CREATE TABLE IF NOT EXISTS cat_wallets (
    ...
    status          TEXT NOT NULL DEFAULT 'free' CHECK (status IN ('free','leased','sweep_pending','frozen')),
...
-- Fast free wallet lookup for LeaseForInvoice
CREATE INDEX IF NOT EXISTS idx_cat_wallets_free_pool
    ON cat_wallets (network_id)
    WHERE status = 'free' AND tier = 'pool' AND is_active = TRUE AND deletion_mark = FALSE;
