...
export function SecurityProfilesSection() {
  ...
  const [profiles, setProfiles] = useTabState<SecurityProfileResponse[]>("sec.profiles", [])
  const hasCachedProfiles = useHasTabCache("sec.profiles")
  ...
  useEffect(() => { if (!hasCachedProfiles) loadProfiles() }, [hasCachedProfiles, loadProfiles])
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex items-center gap-2 flex-wrap">
        ...
        {dimCount > 0 && (
          <Badge variant="outline" className="text-[10px] h-5 font-normal gap-1">
            <span className="font-semibold text-foreground">Доступ к данным (RLS)</span>
            {dimCount} огр.
          </Badge>
        )}
        {flsCount > 0 && (
          <Badge variant="outline" className="text-[10px] h-5 font-normal gap-1">
            <span className="font-semibold text-foreground">Доступность полей (FLS)</span>
            {flsCount} огр.
          </Badge>
        )}
        {ruleCount > 0 && (
          <Badge variant="outline" className="text-[10px] h-5 font-normal gap-1">
            <span className="font-semibold text-foreground">Бизнес правила (CEL)</span>
            {ruleCount} усл.
          </Badge>
        )}
