...
/**
 * PreferencesContent — reusable user preferences form content.
 *
 * Used both in:
 * - UserPreferencesDialog (quick access via avatar menu)
 * - /settings → Профиль section (full settings page)
 *
 * Reads/writes from useUserPrefsStore. Changes auto-save.
 */
...
import type { ThemeMode, AccentColor, DateFormat, NumberFormat } from "@/types/user-prefs"
...
const ACCENT_COLORS: { value: AccentColor; label: string; colorClass: string }[] = [
  { value: "yellow", label: "Жёлтая", colorClass: "bg-yellow-500" },
  { value: "neutral", label: "Нейтральная", colorClass: "bg-slate-500" },
  { value: "blue", label: "Синяя", colorClass: "bg-blue-500" },
]
...
export function PreferencesContent() {
  const { interface: ui, updateInterface } = useUserPrefsStore()
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <h3 className="text-sm font-semibold text-foreground">Оформление</h3>

        <p className="text-xs font-medium text-muted-foreground">Тема</p>
          ...
          {THEMES.map((theme) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex items-center gap-2 rounded-lg border p-2.5 px-3 text-sm transition-colors",
                  isActive
        ...
        <p className="text-xs font-medium text-muted-foreground">Цветовая схема</p>
        <div className="grid grid-cols-3 gap-2">
          {ACCENT_COLORS.map((accent) => {
            const isActive = (ui.accentColor ?? "yellow") === accent.value
            return (
              <button
                key={accent.value}
              ...
              >
                <span className={cn("h-3 w-3 shrink-0 rounded-full", accent.colorClass)} />
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-[1fr_200px] items-start gap-4">
          ...
          <Select
            value={ui.dateFormat ?? "dd.MM.yyyy"}
      ...
      <div className="space-y-4">
        ...
        <SwitchField
          ...
          checked={ui.sidebarCollapsed ?? false}
          onCheckedChange={(v) => update("sidebarCollapsed", v)}
        />
      </div>

      <div className="pt-2 text-xs text-muted-foreground text-center">
        Изменения применяются автоматически к вашей учётной записи
      </div>
    </div>
  )
}
