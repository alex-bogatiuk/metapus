 ...
 *
 * 1. **Pure group** (isGroupOnly=true): table part header like "Products".
...
interface TreeNode extends FilterFieldMeta {
     ...
     * to be visible. E.g. a field inside "Products → Nomenclature" needs
     * ["tp:Products", "ref:items.nomenclatureId"] to both be expanded.
     */
 ...
 *   [+] Organization          (depth=0, selectable + expandable, toggleKey="ref:organizationId")
 *       - Code                (depth=1, leaf, parents=["ref:organizationId"])
 *       - INN                (depth=1, leaf, parents=["ref:organizationId"])
 *   — Supplier's document №   (depth=0, leaf)
 *   [▶] Products              (depth=0, groupOnly, toggleKey="tp:Products")
 *       — Line number         (depth=1, leaf, parents=["tp:Products"])
 *       [+] Nomenclature      (depth=1, selectable+expandable, parents=["tp:Products"], toggleKey="ref:items.nomenclatureId")
 *           - Code            (depth=2, leaf, parents=["tp:Products", "ref:items.nomenclatureId"])
 */
...
export function FilterConfigDialog({
    ...
    function renderNode(node: TreeNode) {
        ...
        // ── Pure group header (e.g. "Products") ──
        if (node.isGroupOnly) {
            return (
                <button
                    key={node.key}
