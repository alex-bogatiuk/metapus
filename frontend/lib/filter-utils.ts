...
/** Operators that accept a range of two values (from / to). */
export function isBetweenOperator(op: ComparisonOperator): boolean {
    return op === "between"
}
    ...
    for (const [key, entry] of Object.entries(values)) {
        ...
        if (key === PERIOD_FILTER_KEY) {
            ...
            if (!meta && key.includes(".")) {
                ...
                if (parentMeta?.refFields) {
                    meta = parentMeta.refFields.find(m => m.key === childKey)
                }
            }
        ...
        // Between operator — value is { from?, to? }, emit two items (gte + lte)
