...
// ── Entity name → Russian label (metadata-driven) ────────────────────────

const ENTITY_LABELS_FALLBACK: Record<string, string> = {
  "*": "Все сущности",
}

function resolveEntityLabel(nameOrKey: string): string {
  if (ENTITY_LABELS_FALLBACK[nameOrKey]) return ENTITY_LABELS_FALLBACK[nameOrKey]
  const store = useMetadataStore.getState()
  // Try by key first (snake_case), then by PascalCase name
  const entity = store.getEntity(nameOrKey) ?? store.getEntityByName(nameOrKey)
  if (entity) return entity.presentation.singular ?? nameOrKey
  return nameOrKey
}
...
export function formatAuditChanges(
  ...
  if (action === "add_rule") {
    ...
    const entity = resolveEntityLabel(String(changes["entityName"] ?? ""))
...
function formatValue(key: string, value: unknown): string {
  ...
  if (key === "entityName" && typeof value === "string") {
    return resolveEntityLabel(value)
  }
