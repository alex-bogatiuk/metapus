...
import { fromQuantity, fromMinorUnits, toQuantity, toMinorUnits, fmtDate } from "@/lib/format"
...
export default function NewGoodsIssuePage() {
  ...
  const { addLine, handlePick: pickLines, handleUpdateField, handleUpdateRef, handleProductSelect, handleUpdateVatRate, handleRemoveLine, handleReorderLines, handleMoveLineUp, handleMoveLineDown, handlePasteLines } = useDocumentLineActions(update, markDirty)
  ...
  const { handleExport: handleExportLines, exporting: exportingLines } = useDocumentLinesExport({
    lines: f.lines,
    documentTitle: `${giLabel} (создание) от ${f.date ? fmtDate(f.date) : ""}`,
