...
import type { GoodsIssueResponse } from "@/types/document"
...
const ALL_COLUMNS: Column<GoodsIssueResponse>[] = [
  {
    key: "date",
    label: "Дата",
    sortable: true,
...
export default function GoodsIssuesListPage() {
  ...
  const storedWidths = useUserPrefsStore((s) => s.getListColumnWidths("GoodsIssue"))
  ...
  const handleWidthsChange = useCallback(
    (widths: Record<string, number>) => setListColumnWidths("GoodsIssue", widths),
    [setListColumnWidths]
  )
  ...
  const [detailDoc, setDetailDoc] = useState<GoodsIssueResponse | null>(null)
  ...
  const refetchAndPatch = useCallback(async (ids: string[]) => {
    const results = await Promise.allSettled(
      ids.map((id) => api.goodsIssues.get(id)),
    )
    const updated = results
      .filter((r): r is PromiseFulfilledResult<GoodsIssueResponse> => r.status === "fulfilled")
      .map((r) => r.value)
    if (updated.length > 0) replaceItems(updated)
  }, [replaceItems])

  // ── Document actions (batch-aware, point-update) ─────────────────
  const handlePostBatch = useCallback(async (docs: GoodsIssueResponse[]) => {
    const toPost = docs.filter((d) => !d.posted && !d.deletionMark)
    if (toPost.length === 0) return
    const ids = toPost.map((d) => d.id)
    try {
      const result = await api.goodsIssues.batchAction(ids, "post")
  ...
  const handleUnpostBatch = useCallback(async (docs: GoodsIssueResponse[]) => {
    const toUnpost = docs.filter((d) => d.posted)
    if (toUnpost.length === 0) return
    const ids = toUnpost.map((d) => d.id)
    try {
      const result = await api.goodsIssues.batchAction(ids, "unpost")
      await refetchAndPatch(ids)
      if (result.failed > 0) {
  ...
  const handleToggleDeletionMarkBatch = useCallback(async (docs: GoodsIssueResponse[], mark: boolean) => {
    const ids = docs.map((d) => d.id)
    const action = mark ? "setDeletionMark" as const : "clearDeletionMark" as const
    try {
      const result = await api.goodsIssues.batchAction(ids, action)
      await refetchAndPatch(ids)
      if (result.failed > 0) {
  ...
  const fetchDetail = useCallback((id: string) => {
    ...
    api.goodsIssues.get(id)
      .then((full) => setDetailDoc(full))
      .catch(() => setDetailDoc(null))
      .finally(() => setDetailLoading(false))
  }, [])

  const handleRowClick = useCallback((doc: GoodsIssueResponse) => {
    setFocusedId(doc.id)
  ...
  const detailsContent = useMemo(() => {
    ...
    return (
      <DocumentDetailsPanel
        title={`${useMetadataStore.getState().getLabel("goods_issue", "singular")} ${detailDoc.number} от ${fmtDate(detailDoc.date)}`}
          ...
          {loading ? (
            ...
            <DataTable
              ...
              onRowDoubleClick={(doc) =>
                router.push(`/documents/goods-issues/${doc.id}`)
              }
              colWidths={colWidths}
              ...
              renderContextMenu={(doc, targets) => {
                ...
                return (
                  <>
                    <ContextMenuItem onClick={() => router.push("/documents/goods-issues/new")}>
                      <Plus className="mr-2 h-4 w-4" />
                      Создать
                      <ContextMenuShortcut>Ins</ContextMenuShortcut>
                    </ContextMenuItem>
                    {!isBatch && (
                      <ContextMenuItem onClick={() => router.push(`/documents/goods-issues/new?copyFrom=${doc.id}`)}>
                    ...
                    {!isBatch && (
                      <ContextMenuItem onClick={() => router.push(`/documents/goods-issues/${doc.id}`)}>
