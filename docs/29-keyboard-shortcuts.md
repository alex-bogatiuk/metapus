...
- **Буквенные клавиши** (`a-z`, `0-9`) — сопоставляются по `event.code` (layout-independent).
  Это решает проблему **кириллических раскладок**: `Ctrl+С` (рус) корректно распознаётся как `Ctrl+C`.
- **Специальные клавиши** (`Enter`, `F9`, `Delete`, `ArrowLeft`) — сопоставляются по `event.key`.
