...
### Посетитель (Visitor) + Рекордер (RegisterRecorder) — мультирегистровые движения

Генерация движений при проведении документов использует паттерн **Visitor** (`posting/visitor.go`).
Запись и реверс движений абстрагированы через **RegisterRecorder** (`posting/recorder.go`).
...
// Интерфейс записи/реверса движений — один на каждый тип регистра
...
// Опциональная предпроверка перед записью (например, наличие остатков)
...
- `CostMovementSource` → движения по себестоимости
- `SettlementMovementSource` → движения по взаиморасчётам

Встроенные рекордеры: `StockRecorder`, `CostRecorder`, `SettlementRecorder` — создаются через `posting.DefaultRecorders()`.

Engine итерирует зарегистрированных посетителей и рекордеров; каждый посетитель проверяет документ через type-assertion.
Расширение: `engine.AddVisitor(&XxxVisitor{})` + `engine.AddRecorder(&XxxRecorder{})` — без изменения существующих документов.
