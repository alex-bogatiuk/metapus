...
### TypedRef — Универсальные составные поля (Polymorphic References)

Аналог `ОписаниеТипов` (составной тип данных) из 1С. Позволяет одному полю ссылаться на разные типы сущностей (документы и справочники). Встраивается в структуры (как правило, строки табличных частей) через embed:

```go
type TypedRef struct {
    RefType string `db:"ref_type" json:"refType"` // EntityName: "GoodsReceipt", "Counterparty"
    RefID   id.ID  `db:"ref_id" json:"refId"`     // UUID сущности
}
```

**Режимы использования:**

1. **Произвольный тип (Любая сущность):**
```go
// Разрешает ссылку на любой зарегистрированный в системе тип
func (e AuditEntry) Validate(ctx context.Context) error {
    return e.TypedRef.ValidateRef(ctx, nil) // nil = any registered entity
}
```

2. **Ограниченный набор типов:**
```go
var BasisTypes = []string{"CashReceipt", "CashPayment"}

func (l BankStatementLine) Validate(ctx context.Context) error {
    // Разрешает только перечисленные типы
    return l.TypedRef.ValidateRef(ctx, BasisTypes)
}
```

**Особенности реализации:**
- Хранится в БД как два поля: `ref_type` (VARCHAR) и `ref_id` (UUID). Требуется создание композитного индекса `(ref_type, ref_id)`.
- Metadata inspector автоматически обнаруживает встроенный `TypedRef` и генерирует поле типа `typed_ref`.
- Для получения представлений (Presentation) ссылок на фронтенде используется batch API-endpoint `POST /api/v1/resolve-refs`. Это решает проблему N+1 запросов при отображении списков.

> [!WARNING] Ограничения: Смешивание ссылок с примитивными типами
> `TypedRef` предназначен **исключительно для ссылок** на другие таблицы. В поле `ref_id` (UUID) физически нельзя сохранить строку (`"Иванов"`), число (`15`) или дату (`2026-03-27`).
> 
> В отличие от 1С (где составной тип может вмещать и `СправочникСсылка.*`, и `Строка`, и `Дата`), в Metapus это **запрещено на уровне `TypedRef`**.
>
> **Как быть, если нужно хранить ЛЮБОЕ значение (Variant)?**
> Если бизнес-задача требует поля, которое принимает и ссылку, и число, и строку (например, План Видов Характеристик, динамические свойства номенклатуры), используйте встроенное поле **`Attributes` (JSONB)** из `BaseEntity`:
> ```go
> // Для сохранения строки:
> doc.Attributes["property_1"] = "Красный"
> // Для сохранения числа:
> doc.Attributes["property_2"] = 10.5
> // Для сохранения ссылки (сохраняем структуру TypedRef):
> doc.Attributes["property_3"] = entity.NewTypedRef("Counterparty", someUUID)
> ```
> База данных PostgreSQL отлично индексирует JSONB поля (через GIN-индексы), что делает `Attributes` идеальным решением для свойств смешанного или «примитивного» типа.
