...
### Шаг 7: Регистрация через CatalogRegistration + FactoryRegistry (1 мин)

**Файл:** `internal/infrastructure/http/v1/catalog_factory.go`

Реализовать интерфейс `CatalogRegistration` (по аналогии с существующими):

```go
type YourEntityRegistration struct{}

func (r *YourEntityRegistration) RoutePrefix() string  { return "your-entities" }
func (r *YourEntityRegistration) Permission() string   { return "catalog:your_entity" }
func (r *YourEntityRegistration) EntityName() string   { return "YourEntity" }
func (r *YourEntityRegistration) EntityLabel() string  { return "Ваша сущность" }
func (r *YourEntityRegistration) ReferenceTypes() []string { return []string{"your_entity"} }
// ... EntityPresentation(), EntityStruct(), Build(deps CatalogDeps) CatalogRouteHandler
```

**Файл:** `internal/infrastructure/http/v1/factory_registry.go`

Добавить 1 строку в `RegisterDefaults()`:
```go
reg.RegisterCatalog(&YourEntityRegistration{})
```
...
> [!TIP]
> **Связанные документы (Preview):**
> Любое поле-ссылка (`id.ID`) из шапки документа автоматически попадет в карточку превью в связанных документах.
> Если поле нужно скрыть из превью, используйте тег `meta:"preview:false"`.
> Подробнее в [20-related-documents.md](20-related-documents.md).
...
#### Использование составных полей (TypedRef)

Если строке документа нужно сослаться на разные типы сущностей (документ-основание, произвольный владелец), используйте `entity.TypedRef`:

```go
var AllowedBasisTypes = []string{"CashReceipt", "CashPayment"}

type YourDocLine struct {
    LineID id.ID `db:"line_id" json:"lineId"`
    // Встраиваем полиморфную ссылку
    entity.TypedRef // добавит refType и refId

    Amount types.MinorUnits `db:"amount" json:"amount"`
}

func (l YourDocLine) Validate(ctx context.Context) error {
    // Валидация: разрешены только конкретные документы (или nil для любой сущности)
    return l.TypedRef.ValidateRef(ctx, AllowedBasisTypes)
}
```

Не забудьте добавить колонки в миграцию БД:
```sql
ref_type VARCHAR(50) NOT NULL,
ref_id   UUID NOT NULL,
-- Рекомендуется индекс для reverse-lookup:
CREATE INDEX ON doc_your_docs_lines (ref_type, ref_id);
```

> [!NOTE] 
> Если вам нужно "составное поле", которое может хранить **и ссылки, и примитивные значения (строку, дату, число)** (например, План Видов Характеристик), используйте `Attributes` (JSONB) вместо `TypedRef`. `TypedRef` поддерживает только UUID и строго ссылки на таблицы. Подробнее в [04-core-layer.md](04-core-layer.md).
...
### Регистрация (через FactoryRegistry)

Документы регистрируются через паттерн **Abstract Factory** (`document_factory.go`) + **FactoryRegistry** (`factory_registry.go`).
...
func (r *YourDocRegistration) EntityName() string  { return "YourDocument" }
func (r *YourDocRegistration) EntityLabel() string { return "Ваш документ" }
func (r *YourDocRegistration) EntityPresentation() metadata.Presentation { ... }
func (r *YourDocRegistration) EntityStruct() interface{} { return &your_doc.YourDocument{} }
...
Затем добавить фабрику в `RegisterDefaults()` (`factory_registry.go`):

```go
reg.RegisterDocument(&YourDocRegistration{})  // ← 1 строка
```

Или, для клиентского расширения (без модификации ядра):
```go
reg := v1.NewFactoryRegistry()
v1.RegisterDefaults(reg)
reg.RegisterDocument(&custom.YourDocRegistration{})  // клиентское расширение
```

Роуты, permissions, metadata и shared dependencies подключатся автоматически через `FactoryRegistry`.
...
- [ ] Регистрация: `DocumentRegistration` в `document_factory.go` + добавить в `RegisterDefaults()` (`factory_registry.go`)
