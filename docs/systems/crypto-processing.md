...
JWT Claims:
  ...
  mrs:  {"019e0879-ce5b-774f-93ec-b613e24ba32d": 1} → роли по мерчантам
```
...
1. Извлекает `MerchantIDs` и роли по мерчантам (`mrs`) из JWT claims
2. Проверяет, что у пользователя есть хотя бы один мерчант
3. Инжектирует `MerchantScope{MerchantIDs, Roles}` в context
4. Все последующие запросы фильтруются по `scope.MerchantIDs`

`RequirePortalRole` применяется только к merchant-bound действиям. Такой запрос обязан содержать явный `merchant_id`, и роль проверяется только для этого мерчанта. Проверка "роль есть хотя бы в одном мерчанте" не используется для write/sensitive операций, чтобы роль Owner/Manager в одном мерчанте не повышала права в другом.
...
Backend:
  ...
  internal/core/context/user.go          — UserContext, MerchantIDs, MerchantRoles
  internal/domain/auth/jwt.go            — JWT claims extension (mids, mrs)
