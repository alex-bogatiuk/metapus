...
`router.go` — связка всех компонентов через **FactoryRegistry** (`factory_registry.go`).

Все сущности (справочники, документы, регистры, отчёты) регистрируются через `FactoryRegistry`:
1. `RegisterDefaults(reg)` — регистрирует все встроенные сущности
2. Клиентские расширения добавляются через `reg.RegisterCatalog()`, `reg.RegisterDocument()`, `reg.RegisterRegister()`, `reg.RegisterReport()`
3. `RouterConfig.Registry` передаётся в `NewRouter()` (nil → автоматически `RegisterDefaults`)

Каждая `CatalogRegistration` / `DocumentRegistration` реализует Abstract Factory: `Build(deps)` → repo → service → handler.

Shared dependencies (`PostingEngine`, `CurrencyResolver`) инжектируются через `RouterConfig` с fallback к defaults.
Документы дополнительно получают shared `PostingEngine`, `CurrencyResolver` и audit hooks через `DocumentDeps`.
