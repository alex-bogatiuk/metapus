...
> **Примечание:** Организационная фильтрация (`orgs`) удалена из JWT. Доступ к организациям управляется исключительно через Security Profiles (RLS dimensions).
...
## Access Scope — авторизация уровня пользователя

`AccessScope` предоставляет базовый контекст авторизации:

```go
scope := security.NewAccessScope(ctx)
// scope.HasPermission(entity, perm) — проверка прав
// scope.RequirePermission(...)      — программная проверка в service layer
```

- **Admin** имеет полный доступ
- Организационная фильтрация выполняется через **DataScope** (Security Profiles), а не через AccessScope
...
Организационный доступ управляется исключительно через Security Profiles.
...
SecurityContextMiddleware
  → BuildDataScope(profileDimensions)
    → DataScope { IsAdmin, Dimensions, EntityDimensions, ReadOnly }

ParseListFilter(c)
  → filter.DataScope = security.GetDataScope(ctx)
    → BaseCatalogRepo / BaseDocumentRepo
      → buildWhereConditions()
        → DataScope.ApplyConditions(entityName, rlsDimensions)
          → WHERE organization_id IN ($1,$2) AND supplier_id IN ($3)
```

### Per-entity RLS dimensions

`EntityDimensions` позволяет задавать ограничения для конкретной сущности, переопределяя глобальные:

```go
DataScope{
    Dimensions: map[string][]string{
        "organization": {"org-1", "org-2"},  // глобально
    },
    EntityDimensions: map[string]map[string][]string{
        "goods_receipt": {
            "organization": {"org-1"},  // для goods_receipt — только org-1
        },
    },
}
```

В БД: столбец `entity_name` в `security_profile_dimensions`:
- `entity_name = ''` → глобальное измерение (все сущности)
- `entity_name = 'goods_receipt'` → только для этой сущности
...
- Вызывает `scope.CanAccessRecord(entityName, doc.GetRLSDimensions())`
...
security_profile_dimensions    — RLS: profile_id, dimension_name, entity_name, allowed_ids UUID[]
...
type SecurityProfile struct {
    ...
    Dimensions       map[string][]string                  // RLS — глобальные (entity_name="")
    EntityDimensions map[string]map[string][]string       // RLS — per-entity
    FieldPolicies    map[string]*security.FieldPolicy     // FLS (key: "entity:action")
    PolicyRules      []*PolicyRule                        // CEL rules
}
```
...
4. Строит `DataScope` из profile dimensions (профиль — единственный источник org-ограничений)
5. Инжектирует `security.WithDataScope(ctx, scope)` + `security.WithFieldPolicies(ctx, policies)` + `security.WithPolicyRules(ctx, rules)`

### Fail-open / Fail-closed

- **Нет профиля** → fail-open: пустой `DataScope{}` = полный доступ, без FLS
- **Ошибка загрузки профиля** → fail-closed: restrictive scope (пустые allowed IDs = нет доступа)
- **Невалидный UserID** → fail-closed
...
| `user` | `map<string, any>` | `{id, email, roles, isAdmin}` (orgIds deprecated — всегда `[]`) |
